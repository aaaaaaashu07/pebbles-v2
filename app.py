...
def init_db():
    ...
    conn.execute('''CREATE TABLE IF NOT EXISTS users (
        ...
        password TEXT NOT NULL, phone TEXT, flat_no TEXT, tower TEXT,
        role TEXT DEFAULT 'resident'
    )''')

    conn.execute('''CREATE TABLE IF NOT EXISTS vehicles (
        id INTEGER PRIMARY KEY, user_id INTEGER NOT NULL,
        plate TEXT NOT NULL UNIQUE, label TEXT, type TEXT DEFAULT 'car',
        FOREIGN KEY(user_id) REFERENCES users(id)
    )''')

    conn.execute('''CREATE TABLE IF NOT EXISTS parking_slots (
        id INTEGER PRIMARY KEY, slot_number TEXT UNIQUE NOT NULL,
        tower TEXT, zone TEXT, slot_type TEXT DEFAULT 'car',
        floor_level TEXT DEFAULT 'Ground', status TEXT DEFAULT 'free',
        maintenance INTEGER DEFAULT 0, booked_by INTEGER,
        vehicle_no TEXT, booked_at TEXT, expires_at TEXT,
        FOREIGN KEY(booked_by) REFERENCES users(id)
    )''')

    conn.execute('''CREATE TABLE IF NOT EXISTS bookings (
        id INTEGER PRIMARY KEY, user_id INTEGER, slot_id INTEGER,
        slot_number TEXT, tower TEXT, zone TEXT, slot_type TEXT,
        vehicle_no TEXT, booked_at TEXT, expires_at TEXT,
        released_at TEXT, status TEXT DEFAULT 'active',
        FOREIGN KEY(user_id) REFERENCES users(id)
    )''')

    # Seed admin — safe upsert
    existing_admin = conn.execute("SELECT id FROM users WHERE email='admin@pebbles.com'").fetchone()
    ...
    # Seed guard
    existing_guard = conn.execute("SELECT id FROM users WHERE email='guard@pebbles.com'").fetchone()
...
def api_book():
    ...
    exp_lbl = expires.strftime('%d %b, %I:%M %p')
    ...
    conn.commit()
    conn.close()
    return jsonify({'success':True,'message':f"✅ {slot['slot_number']} booked! Expires {exp_lbl}"})
